...
class RetinaNetBBPredictor(tf.keras.Model):
    def __init__(self, width: int, depth: int, num_anchors: int = 9, prefix: str = ''):
        ...
        self.feature_extractors = [layers.ConvBlock(width, kernel_size=3, activation='swish', padding='same', separable=True, pointwise_initializer=tf.initializers.VarianceScaling(), depthwise_initializer=tf.initializers.VarianceScaling(), prefix=prefix + f'conv_block_{i}/')
    ...
    def call(self, features: tf.Tensor, training: bool = True) -> tf.Tensor:
        ...
        x = tf_utils.cascade_layers(self.feature_extractors, features, training=training)
...
class RetinaNetClassifier(tf.keras.Model):
    ...
    def __init__(self, width: int, depth: int, num_classes: int, num_anchors: int = 9, prefix: str = ''):
        ...
        self.feature_extractors = [
            layers.ConvBlock(width, kernel_size=3, activation='swish', padding='same', prefix=prefix + f'conv_block_{i}/', separable=True, pointwise_initializer=tf.initializers.VarianceScaling(), depthwise_initializer=tf.initializers.VarianceScaling(), bias_initializer='zeros')
            for i in range(depth)]
